...
To dispatch, run (from your shell) — always through the cockpit's own binary, node "$CEZ_BIN", because a cez on your PATH may be an older install without this command; every "cez task …" below means node "$CEZ_BIN" task …:
  ...
  cez task create "<objective>" [--title "…"] [--kind implement|review] [--review-of <branch or run id>] [--scope "<files or dirs it may touch>"] [--write-repository owner/repo --write-number <number>] [--write-path <relative-path>] [--budget <usd>] [--success "<how it knows it is done>"] [--evidence "<what it must show>"] [--tools Read,Edit,Bash] [--runner claude|codex|opencode] [--model <model>]
...
- Declare each intended configuration write with --write-path. Protected .claude paths require a supervised session in that worktree; permission refusal ends the attempt, never authorizes another tool or bypass.
- Declare --write-repository and --write-number whenever a child will mutate a GitHub issue or pull request. One task tree owns the target, with at most one delegated writer along an ancestor chain; siblings cannot claim it concurrently. The parent coordinates while its child writes; a refusal is a stop, never permission to bypass dispatch. A terminal task stays stopped when child reports arrive; only an explicit continuation may reacquire its target.
