...
**A substring match and a success banner are not evidence; read the field the system writes.** The rule above covers a claim made without looking. This one covers the harder case: the artifact was in hand and the reading was still wrong. Searching a killed run's NDJSON for `total_cost` returned four hits, which became a published comment on #15 asserting the usage telemetry had arrived and only the reconciliation into the record was skipped. The hits were `item.completed` and `tool-result` events whose *content* was the source of `scripts/mock-claude.mjs`, a file the agent had opened with a Read tool while it worked - one match being the literal `total_cost_usd: 0` from the mock's own `mock:auth-error` branch. The authoritative reading is each event's `type`: across all 217 lines there is no `cost`, no `token-usage`, no `turn.completed`, and one `turn.started` with no completion, so no telemetry ever arrived and there was nothing to reconcile. The comment had to be retracted. The shell form of the same error twice in the same session: `${PIPESTATUS[0]}` read after an intervening command printed an empty exit status, and `✓ build ok` on stdout was nearly reported as a passing build on that basis. Capture an exit status before anything else runs - or redirect output to a file and read the status first - and when a search matches, confirm which field matched before concluding anything from it.
